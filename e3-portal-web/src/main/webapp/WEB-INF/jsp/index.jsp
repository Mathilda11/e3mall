...
<div class="slide_show" id="slide_show">
	<div class="indexW">
		...
		<ol>
			...
			<li>
			  <a name="sfbest_hp_hp_focus_${status.index }" class="fore_pic trackref" href="${node.url }" target="_blank"> 
				<img id="lunbo_1" alt="${node.sub_title }"	src="${node.pic }">
			  </a>
			  </li>
			</c:forEach>
			<!-- <li>
     ...
     <div class="rSide">
     	<c:forEach items="${ad1List}" var="node" varStatus="status">
			<li>
		           <a name="sfbest_hp_hp_focus_right-ad1" class="a-img r-img1 trackref" href="${node.url }" target="_blank">
          <img alt="${node.titleDesc }" src="${node.pic }">
          <div class="rmask"></div>
          </a></li>
          </c:forEach>
				...
				<c:forEach items="${ad1List}" var="node" varStatus="status">
					<li class="price_list1" eid="l_218031_7_297" goods="218031"
						id="cx_l_218031_7_297"><div class="pImg">
							<a
								href="${node.url }"
								target="_blank" title="${node.title }"><img class="lazy"
								data="${node.pic }"
								alt="${node.titleDesc }"
								src="${node.pic }"
						...
						<div class="title-a">
							<a
								href="${node.url }"
								target="_blank" title="${node.title }">${node.title }</a>
						</div>
